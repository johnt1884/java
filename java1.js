...
let newReplyIds = new Set(JSON.parse(localStorage.getItem('otkNewReplyIds') || '[]'));
...
function appendTextOrQuoteSegment(textElement, segment, quoteRegex, currentDepth, MAX_QUOTE_DEPTH, messagesByThreadId, uniqueImageViewerHashes, boardForLink, mediaLoadPromises, parentMessageId) {
    ...
    if (quoteMatch && segment.startsWith(quoteMatch[0])) { // Process as quote only if segment starts with it
        ...
        if (quotedMessageObject) {
            ...
            let quoteLinkColor = '#88ccee';
            try {
                const settings = JSON.parse(localStorage.getItem('otkThemeSettings')) || {};
                if (settings.otkQuoteLinkColor) {
                    quoteLinkColor = settings.otkQuoteLinkColor;
                }
            } catch (e) {}
            notFoundSpan.style.color = quoteLinkColor;
...
function createTweetEmbedElement(tweetId) {
    ...
    if (!otkGuiWrapper) {
        ...
        scrollButtonContainer.id = 'otk-scroll-button-container';
        scrollButtonContainer.style.cssText = `
            display: none; /* Hidden by default */
            flex-direction: column;
            align-items: center;
            justify-content: center;
        ...
        scrollTopButton.id = 'otk-scroll-top-btn';
        scrollTopButton.innerHTML = '&#9650;'; // Chevron Up
        ...
        scrollBottomButton.id = 'otk-scroll-bottom-btn';
        scrollBottomButton.innerHTML = '&#9660;'; // Chevron Down
        ...
        if (typeof updateViewerTitle === 'function') {
            updateViewerTitle();
        } else {
...
function createThreadListItemElement(thread, isForTooltip = false) {
    ...
    if (!isForTooltip) {
        ...
        titleLink.onclick = (event) => {
            ...
            if (scrollButtonContainer) scrollButtonContainer.style.display = 'flex';
            setTimeout(() => {
...
function renderThreadList() {
    ...
    async function renderMessagesInViewer(options = {}) {
        const { isToggleOpen = false, showLoading = true } = options;
        if (!otkViewer) {
        ...
        const currentMode = localStorage.getItem('otkViewerMode') || 'closed';
        if (currentMode === 'messages') {
            const matchedIds = new Set();
            const keywordsStr = localStorage.getItem('otkTrackedKeywordsMessages') || "otk";
            const keywords = keywordsStr.split(',').map(k => k.trim().toLowerCase()).filter(k => k.length > 0);

            // First pass: Find all seed messages containing any of the keywords
            ...
            // Second pass: Find subsequent messages that reply directly or indirectly to a matched message
            // Since allMessages is sorted by timestamp/ID ascending, we propagate matchedIds.
            for (const msg of allMessages) {
                if (msg.text) {
                    const quoteRegex = />>(\d+)/g;
                    let match;
                    while ((match = quoteRegex.exec(msg.text)) !== null) {
                        const quotedId = parseInt(match[1], 10);
                        if (matchedIds.has(quotedId)) {
                            matchedIds.add(msg.id);
                            break;
                        }
                    }
            ...
            allMessages = allMessages.filter(msg => matchedIds.has(msg.id));
            consoleLog(`[MessageTracker] Filtered to ${allMessages.length} messages matching keywords or subsequent replies.`);
        }
...
updateDisplayedStatistics(false); // Update stats after all media processing is attempted.
    ...
    async function appendNewMessagesToViewer(newMessages) {
        ...
        const currentMode = localStorage.getItem('otkViewerMode') || 'closed';
        if (currentMode === 'messages') {
            consoleLog("[appendNewMessagesToViewer] Message Tracker mode active. Re-rendering entire view for correctness.");
            await renderMessagesInViewer({ showLoading: false });
            return;
        }
    ...
    function handleScrollRead() {
        ...
        if (newestVisibleUnreadMessage) {
            ...
            const unreadToRemove = new Set();
            for (const id of unreadIds) {
                const message = allMessages.find(m => m.id === id);
                if (message && message.time > cutoffTime) {
                    newUnreadIds.add(id);
                } else {
                    unreadToRemove.add(id);
                    unreadChanged = true;
                }
            }

            let repliesChanged = false;
            const newNewReplyIds = new Set();
            const repliesToRemove = new Set();
            for (const id of newReplyIds) {
                const message = allMessages.find(m => m.id === id);
                if (message && message.time > cutoffTime) {
                    newNewReplyIds.add(id);
                } else {
                    repliesToRemove.add(id);
                    repliesChanged = true;
                }
            }

            if (unreadChanged) {
                consoleLog(`[ScrollRead] Marking ${unreadToRemove.size} messages as read.`);
                unreadIds = newUnreadIds;
                unreadToRemove.forEach(id => {
            ...
            if (repliesChanged) {
                consoleLog(`[ScrollRead] Marking ${repliesToRemove.size} replies as seen.`);
                newReplyIds = newNewReplyIds;
                localStorage.setItem('otkNewReplyIds', JSON.stringify(Array.from(newReplyIds)));
            }

            if (unreadChanged || repliesChanged) {
                updateDisplayedStatistics();
            }
        }
    }
...
function _populateMessageBody(message, mediaLoadPromises, uniqueImageViewerHashes, boardForLink, isTopLevelMessage, currentDepth, threadColor, parentMessageId, ancestors, allThemeSettings, shouldDisableUnderline, effectiveDepthForStyling) {
    const quoteLinkColor = (allThemeSettings && allThemeSettings.otkQuoteLinkColor) ? allThemeSettings.otkQuoteLinkColor : '#88ccee';
    const directQuoteIds = [];
    const textElement = document.createElement('div');
    textElement.className = 'otk-message-body-text';
    textElement.style.whiteSpace = 'pre-wrap';
    textElement.style.overflowWrap = 'break-word';
    textElement.style.wordBreak = 'normal';

    // Always ensure no margin/padding at the start of textElement
    textElement.style.marginTop = '0px';
    textElement.style.paddingTop = '0px';
    if (shouldDisableUnderline) {
        textElement.style.paddingBottom = '0px';
    }

    let msgText = message.text || '';
    // Ensure spacing settings override any spacing within an actual message at its start
    msgText = msgText.trimStart();
    ...
    if (msgText && typeof msgText === 'string') {
                ...
                while (currentTextSegment.length > 0) {
                    ...
                    if (earliestMatch) {
                        ...
                        if (earliestMatchIsQuoteLink) {
                            const quotedMessageId = Number(earliestMatch[1]);
                            if (currentDepth < 2) {
                                if (!directQuoteIds.includes(quotedMessageId)) {
                                    directQuoteIds.push(quotedMessageId);
                                }
                            }
                            ...
                            for (const threadIdKey in messagesByThreadId) {
                                if (messagesByThreadId.hasOwnProperty(threadIdKey)) {
                                    const foundMsg = messagesByThreadId[threadIdKey].find(m => m.id === quotedMessageId);
                                    if (foundMsg) {
                                        quotedMessageObject = foundMsg;
                                        break;
                                    }
                            ...
                            quoteSpan.className = "otk-inline-quote-link";
                            quoteSpan.dataset.quotedId = quotedMessageId;
                            quoteSpan.style.color = quoteLinkColor;
        ...
        textElement.textContent = msgText;
    }

    // Clean up any leading empty text/whitespace nodes or <br> tags from textElement to ensure spacing settings override any spacing within an actual message at its start
    while (textElement.firstChild) {
        const first = textElement.firstChild;
        if (first.nodeType === 3 && first.textContent.trim() === '') {
            textElement.removeChild(first);
        } else if (first.nodeType === 1 && first.tagName === 'BR') {
            textElement.removeChild(first);
        } else {
            break;
        }
    ...
    return [textElement, attachmentDiv, directQuoteIds];
}
...
function _createMessageHeaderIcons(message, messageDiv, isFiltered, headerContainer) {
    ...
    let hoverPlusIcon = null;
    const isTopLevel = messageDiv.classList.contains('otk-message-container-main');
    if (!isTopLevel && hasTruncatedQuotes(message)) {
        hoverPlusIcon = document.createElement('span');
        hoverPlusIcon.className = 'otk-hover-plus-icon';
        hoverPlusIcon.textContent = '+';
        hoverPlusIcon.title = 'Load next reply in truncated chain';
        hoverPlusIcon.style.cssText = `
            margin-left: 8px;
            cursor: pointer;
            font-weight: bold;
            font-size: 16px;
            ...
            visibility: hidden;
            color: var(--otk-pin-icon-color-odd, #aaa);
        `;

        hoverPlusIcon.addEventListener('click', async (e) => {
            e.stopPropagation();
            ...
            if (!messageToLoad) {
                ...
                try {
                    ...
                    clickedPlusIcon.style.color = 'var(--otk-pin-icon-color-odd, #aaa)';
                }
            }
            ...
            if (newElement) {
                let allThemeSettingsTemp = {};
                try {
                    allThemeSettingsTemp = JSON.parse(localStorage.getItem('otkThemeSettings')) || {};
                } catch (err) {}
                const ensurePxTemp = (val, defaultVal) => {
                    if (val === undefined || val === null || val === '') return defaultVal;
                    const s = String(val).trim();
                    if (/^\d+$/.test(s)) return s + 'px';
                    return s;
                };
                const otkNewMessageSpaceBelowQuoteBlockTemp = ensurePxTemp(allThemeSettingsTemp.otkNewMessageSpaceBelowQuoteBlock, '11px');
                const otkQuotedOuterPaddingTemp = ensurePxTemp(allThemeSettingsTemp.otkQuotedOuterPadding, '11px');
                const otkQuotedDividerSpacingTemp = ensurePxTemp(allThemeSettingsTemp.otkQuotedDividerSpacing, '5px');
                const quotedMessageDividerColorTemp = allThemeSettingsTemp.otkQuotedMessageDividerColor || '#ff8040';
                const quotedMessageDividerStyleTemp = allThemeSettingsTemp.otkQuotedMessageDividerStyle || 'dashed';

                let quotesContainer = topLevelElement.querySelector('.otk-quotes-container-block');
                if (!quotesContainer) {
                    quotesContainer = document.createElement('div');
                    quotesContainer.className = 'otk-quotes-container-block';
                    ...
                    if (topLevelElement.firstChild && topLevelElement.firstChild.nextSibling) {
                        topLevelElement.insertBefore(quotesContainer, topLevelElement.firstChild.nextSibling);
                    } else {
                        topLevelElement.appendChild(quotesContainer);
                    }
                }
                ...
                newElement.style.paddingRight = '0px';
                newElement.style.width = '100%';
                newElement.style.maxWidth = '100%';

                const divider = document.createElement('div');
                divider.style.cssText = `
                    border-top: 2px ${quotedMessageDividerStyleTemp} ${quotedMessageDividerColorTemp};
                    margin-top: ${otkQuotedDividerSpacingTemp};
                    margin-bottom: ${otkQuotedDividerSpacingTemp};
                    margin-left: 0;
                    margin-right: 0;
                    width: 100%;
                `;

                if (insertionParent && insertionParent.parentNode === quotesContainer) {
                    quotesContainer.insertBefore(newElement, insertionParent);
                    quotesContainer.insertBefore(divider, insertionParent);
                } else {
                    quotesContainer.appendChild(divider);
                    quotesContainer.appendChild(newElement);
                }

                if (!findNextUnloadedQuoteLink(topLevelElement)) {
                    clickedPlusIcon.remove();
                    const rightPlus = topLevelElement.querySelector('.otk-plus-icon');
                    if (rightPlus) {
                        rightPlus.remove();
                    }
                }
        ...
        headerContainer.appendChild(hoverPlusIcon);
    }
    ...
    headerContainer.addEventListener('mouseenter', () => {
        ...
        if (hoverPlusIcon) {
            hoverPlusIcon.style.visibility = 'visible';
        }
    ...
    headerContainer.addEventListener('mouseleave', () => {
        ...
        if (hoverPlusIcon) {
            hoverPlusIcon.style.visibility = 'hidden';
        }
        if (!multiQuoteWrapper.classList.contains('selected')) {
...
function hideQuoteLinkAndCleanupNewline(textElement, quoteId) {
    if (!textElement) return;
    const link = textElement.querySelector(`.otk-inline-quote-link[data-quoted-id='${quoteId}']`);
    if (link) {
        link.style.display = 'none';

        // Remove any immediately following BR tag or empty text nodes
        ...
        // Also check if there is an immediately preceding empty text node
...
function createMessageElementDOM(message, mediaLoadPromises, uniqueImageViewerHashes, boardForLink, isTopLevelMessage, currentDepth, threadColor, parentMessageId = null, ancestors = new Set(), visualDepth = null) {
        ...
        const otkNewMessagePaddingTop = ensurePx(allThemeSettings.otkNewMessagePaddingTop, '5px');
        const otkNewMessageSpaceBelowQuoteBlock = ensurePx(allThemeSettings.otkNewMessageSpaceBelowQuoteBlock, '11px');
        const otkNewMessagePaddingBottom = ensurePx(allThemeSettings.otkNewMessagePaddingBottom, '11px');
        const otkQuotedOuterPadding = ensurePx(allThemeSettings.otkQuotedOuterPadding, '11px');
        const otkQuotedInternalPadding = ensurePx(allThemeSettings.otkQuotedInternalPadding, '5px');
        const otkQuotedHeaderSpacing = ensurePx(allThemeSettings.otkQuotedHeaderSpacing, '1px');
        const otkQuotedDividerSpacing = ensurePx(allThemeSettings.otkQuotedDividerSpacing, '5px');
        const quotedMessageDividerColor = allThemeSettings.otkQuotedMessageDividerColor || '#ff8040';
        const quotedMessageDividerStyle = allThemeSettings.otkQuotedMessageDividerStyle || 'dashed';
    ...
    if (borderColorVar !== 'transparent') {
        messageDiv.style.border = `1px solid ${borderColorVar}`;
            ...
            messageHeader.style.cssText = `
                ...
                margin-bottom: ${otkNewMessagePaddingTop};
                padding-bottom: 5px;
            ...
            if (shouldDisableUnderline) {
                ...
                messageHeader.style.marginBottom = otkQuotedHeaderSpacing;
            ...
            if (isTopLevelMessage) {
                ...
                if (hasTruncatedQuotes(message)) { // Use the recursive check for initial display
                    ...
                    plusIcon.title = 'Load all messages in quote chain';
                        ...
                        try {
                            while (true) {
                                ...
                                if (newElement) {
                                    ...
                                    const divider = document.createElement('div');
                                    divider.style.cssText = `
                                        border-top: 2px ${quotedMessageDividerStyle} ${quotedMessageDividerColor};
                                        margin-top: ${otkQuotedDividerSpacing};
                                        margin-bottom: ${otkQuotedDividerSpacing};
                                        margin-left: 0;
                                        margin-right: 0;
                                        width: 100%;
                                    `;
                                    ...
                                    const textBody = insertionParent.querySelector('.otk-message-body-text') || insertionParent;
                                    hideQuoteLinkAndCleanupNewline(textBody, messageToLoadId);
                                }
                            }
                        } finally {
                            clickedPlusIcon.style.cursor = 'pointer';
                            ...
                            if (!nextTruncatedInfo) {
                                clickedPlusIcon.remove();
                                const hoverPlus = topLevelElement.querySelector('.otk-hover-plus-icon');
                                if (hoverPlus) {
                                    hoverPlus.remove();
                                }
                            }
            ...
            if (isTopLevelMessage && typeof directQuoteIds !== 'undefined' && directQuoteIds && directQuoteIds.length > 0) {
                ...
                if (existingQuotedIds.length > 0) {
                    ...
                    if (parentMsgObj) {
                        ...
                        let grandparentMsgObj = null;
                        if (parentMsgObj.text && typeof parentMsgObj.text === 'string') {
                            const gpIds = [];
                            const gpRegex = />>(\d+)/g;
                            let gpMatch;
                            while ((gpMatch = gpRegex.exec(parentMsgObj.text)) !== null) {
                                const gpId = Number(gpMatch[1]);
                                if (!gpIds.includes(gpId)) {
                                    gpIds.push(gpId);
                                }
                            }
                        ...
                        // Now build the single quotesContainer
                        messageHeader.style.marginBottom = '10px'; // spacing under underline to grey block
                        quotesContainer = document.createElement('div');
                        quotesContainer.className = 'otk-quotes-container-block';
                        quotesContainer.style.cssText = `
                            background-color: var(--otk-msg-depth-even-bg-color, #ececec);
                        ...
                        // Hide the direct quote link from Level 0 message body since it's shown in the block
                        if (parentMsgObj) {
                            hideQuoteLinkAndCleanupNewline(textElement, parentMsgObj.id);
                        }

                        // Render grandparent first (if it exists)
                        if (grandparentMsgObj) {
                            const gpElement = createMessageElementDOM(
                                grandparentMsgObj,
                                mediaLoadPromises,
                                uniqueImageViewerHashes,
                                grandparentMsgObj.board || boardForLink,
                                false,
                                2, // currentDepth = 2
                                null,
                                parentMsgObj.id,
                                newAncestors,
                                visualDepth !== null ? visualDepth + 2 : 2
                            );
                            if (gpElement) {
                                gpElement.style.backgroundColor = 'transparent';
                                gpElement.style.border = 'none';
                                gpElement.style.boxShadow = 'none';
                                gpElement.style.marginTop = '0px';
                                gpElement.style.marginBottom = '0px';
                                gpElement.style.marginLeft = '0px';
                                gpElement.style.paddingTop = '0px';
                                gpElement.style.paddingLeft = '0px';
                                gpElement.style.paddingRight = '0px';
                                gpElement.style.width = '100%';
                                gpElement.style.maxWidth = '100%';
                                quotesContainer.appendChild(gpElement);
                            }
                        }

                        // Render parent second
                        if (parentMsgObj) {
                            if (quotesContainer.children.length > 0) {
                                const divider = document.createElement('div');
                                divider.style.cssText = `
                            ...
                            const pElement = createMessageElementDOM(
                                parentMsgObj,
                                mediaLoadPromises,
                                uniqueImageViewerHashes,
                                parentMsgObj.board || boardForLink,
                                false,
                                1, // currentDepth = 1
                                null,
                                message.id,
                                newAncestors,
                                visualDepth !== null ? visualDepth + 1 : 1
                            );
                            if (pElement) {
                                pElement.style.backgroundColor = 'transparent';
                                pElement.style.border = 'none';
                                pElement.style.boxShadow = 'none';
                                pElement.style.marginTop = '0px';
                                pElement.style.marginBottom = '0px';
                                pElement.style.marginLeft = '0px';
                                pElement.style.paddingTop = '0px';
                                pElement.style.paddingLeft = '0px';
                                pElement.style.paddingRight = '0px';
            ...
            if (quotesContainer) {
                messageDiv.appendChild(quotesContainer);
            }
            ...
            if (isFiltered) {
                ...
                if (!hasUnfilteredContent && !hasQuotes) {
                    ...
                    const [originalTextElement, originalAttachmentDiv, originalDirectQuoteIds] = _populateMessageBody(message, mediaLoadPromises, uniqueImageViewerHashes, boardForLink, isTopLevelMessage, currentDepth, threadColor, parentMessageId, newAncestors, allThemeSettings, shouldDisableUnderline, effectiveDepthForStyling);
                    ...
                    if (pinIcon) { // pinIcon is the SVG element
                        ...
                        showFilteredLink.addEventListener('click', (e) => {
                            ...
                            if (!originalBodyGenerated) {
                                const [originalTextElement, originalAttachmentDiv, originalDirectQuoteIds2] = _populateMessageBody(message, mediaLoadPromises, uniqueImageViewerHashes, boardForLink, isTopLevelMessage, currentDepth, threadColor, parentMessageId, newAncestors, allThemeSettings, shouldDisableUnderline, effectiveDepthForStyling);
...
async function backgroundRefreshThreadsAndMessages(options = {}) { // Added options parameter
                    ...
                    if (message.text) {
                        ...
                        for (const quote of quotes) {
                            ...
                            if (userPostIds.has(quoteId)) {
                                newReplyIds.add(message.id);
                                break; // Only count one reply per message
                            }
                        }
                    }
                });
                localStorage.setItem('otkNewReplyIds', JSON.stringify(Array.from(newReplyIds)));
                consoleLog(`[BG] Found ${newReplyIds.size} new replies to user posts.`);
            }
    ...
    localStorage.setItem(THREADS_KEY, JSON.stringify(activeThreads)); // activeThreads is already updated by catalog sync
            ...
            localStorage.setItem('otkNewReplyIds', JSON.stringify([]));
            consoleLog(`[Manual Refresh] Cleared unread message and new reply lists.`);
    ...
    async function clearAndRefresh() {
        ...
        try {
            ...
            localStorage.removeItem('otkNewReplyIds');
...
const scrollButtonContainer = document.getElementById('otk-scroll-button-container');
    ...
    function updateToggleButtonText() {
        const btnToggleViewer = document.getElementById('otk-toggle-viewer-btn');
        if (!btnToggleViewer) return;
        const currentMode = localStorage.getItem('otkViewerMode') || 'closed';
        if (currentMode === 'closed') {
            btnToggleViewer.textContent = 'Toggle Viewer';
        } else if (currentMode === 'default') {
            btnToggleViewer.textContent = 'Message Tracker';
        } else if (currentMode === 'messages') {
            btnToggleViewer.textContent = 'Remove Viewer';
        }
    }
    ...
    function toggleViewer() {
        ...
        let currentMode = localStorage.getItem('otkViewerMode') || 'closed';

        if (currentMode === 'closed') {
            // Transition: Closed -> Default Mode
            currentMode = 'default';
            localStorage.setItem('otkViewerMode', 'default');
            ...
            // Transition: Default Mode -> Message Tracker Mode
            currentMode = 'messages';
            localStorage.setItem('otkViewerMode', 'messages');
            localStorage.setItem(VIEWER_OPEN_KEY, 'true');
            ...
            updateToggleButtonText();
            updateViewerTitle();
            renderMessagesInViewer({isToggleOpen: false});
        } else if (currentMode === 'messages') {
            // Transition: Message Tracker Mode -> Closed
            currentMode = 'closed';
            localStorage.setItem('otkViewerMode', 'closed');
            ...
            if (scrollButtonContainer) scrollButtonContainer.style.display = 'none';
...
function applyThemeSettings(options = {}) {
    ...
    // Clock Colors
        ...
        if (settings.repliesStatColor) {
            document.documentElement.style.setProperty('--otk-replies-stat-color', settings.repliesStatColor);
            updateColorInputs('replies-stat', settings.repliesStatColor);
        }
    }
...
function createThemeOptionRow(options) {
    // options = { labelText, storageKey, cssVariable, defaultValue, inputType, unit, min, max, step, idSuffix, requiresRerender }
    const group = document.createElement('div');
    group.classList.add('otk-option-row');
    ...
    mainInput.type = options.inputType || 'text';
    ...
    if (options.inputType === 'color') {
        ...
        hexInput.style.cssText = `
            ...
            text-align: right;
        `;
        mainInput.style.flexGrow = '0';
        mainInput.style.flexShrink = '0';
        mainInput.style.width = '30px';
        mainInput.style.padding = '1px';
    }
    ...
    const updateInputs = (value) => {
        ...
        if (options.unit && typeof displayValue === 'string' && displayValue.endsWith(options.unit)) {
            displayValue = displayValue.replace(options.unit, '');
        }
        mainInput.value = displayValue;
        if (hexInput) {
    ...
    updateInputs(initialValue);

    const saveValue = (newValue) => {
        let valueToSave = newValue;
        if (options.unit && !String(newValue).endsWith(options.unit)) {
            valueToSave += options.unit;
        }
        saveThemeSetting(options.storageKey, valueToSave, options.requiresRerender || false);
        if (options.cssVariable) {
            document.documentElement.style.setProperty(options.cssVariable, valueToSave);
        }
    };
    ...
    defaultBtn.addEventListener('click', () => {
        updateInputs(options.defaultValue);
        saveValue(options.defaultValue);
        // After setting to default, re-apply all theme settings to ensure consistency
        applyThemeSettings({ forceRerender: false });
    });
    ...
    function setupOptionsWindow() {
        ...
        let themeOptionsContainer = null;

        const createCollapsibleSubSection = (title, { isH6 = false, defaultCollapsed = true, parent = null } = {}) => {
            ...
            const targetParent = parent || themeOptionsContainer || document.getElementById('otk-theme-options-container');
            if (targetParent) {
        ...
        themeSection.style.cssText = `
            ...
            box-sizing: border-box;
        `;
        themeOptionsContainer = document.createElement('div');
        ...
        const generalSettingsSection = createCollapsibleSubSection('General Settings', { defaultCollapsed: true });

        // --- Tracked Keywords (Threads) Option ---
        ...
        trackedKeywordsLabel.textContent = "Tracked Keywords (Threads):";
        ...
        // --- Tracked Keywords (Messages) Option ---
        const trackedKeywordsMessagesGroup = document.createElement('div');
        trackedKeywordsMessagesGroup.classList.add('otk-option-row');

        const trackedKeywordsMessagesLabel = document.createElement('label');
        trackedKeywordsMessagesLabel.textContent = "Tracked Keywords (Messages):";
        trackedKeywordsMessagesLabel.htmlFor = 'otk-tracked-keywords-messages-input';
        trackedKeywordsMessagesLabel.style.cssText = "font-size: 12px; text-align: left;";

        const trackedKeywordsMessagesControlsWrapper = document.createElement('div');
        trackedKeywordsMessagesControlsWrapper.style.cssText = "display: flex; align-items: center; gap: 8px;";

        const trackedKeywordsMessagesInput = document.createElement('input');
        trackedKeywordsMessagesInput.type = 'text';
        trackedKeywordsMessagesInput.id = 'otk-tracked-keywords-messages-input';
        trackedKeywordsMessagesInput.placeholder = "e.g., otk, keyword2, phrase three";
        trackedKeywordsMessagesInput.style.cssText = "width: 100%; height: 25px; box-sizing: border-box; font-size: 12px; text-align: right;";
        trackedKeywordsMessagesInput.value = localStorage.getItem('otkTrackedKeywordsMessages') || "otk";

        trackedKeywordsMessagesInput.addEventListener('change', () => {
            const valueToSave = trackedKeywordsMessagesInput.value.trim();
            if (valueToSave) {
                localStorage.setItem('otkTrackedKeywordsMessages', valueToSave);
                consoleLog(`Tracked keywords (messages) saved: ${valueToSave}`);
            } else {
                localStorage.setItem('otkTrackedKeywordsMessages', "otk");
                trackedKeywordsMessagesInput.value = "otk";
                consoleLog(`Tracked keywords (messages) reset to default: "otk"`);
            }
            const currentMode = localStorage.getItem('otkViewerMode') || 'closed';
            if (currentMode === 'messages' && otkViewer && otkViewer.style.display === 'block') {
                renderMessagesInViewer();
            }
        });

        trackedKeywordsMessagesControlsWrapper.appendChild(trackedKeywordsMessagesInput);
        trackedKeywordsMessagesGroup.appendChild(trackedKeywordsMessagesLabel);
        trackedKeywordsMessagesGroup.appendChild(trackedKeywordsMessagesControlsWrapper);
        generalSettingsSection.appendChild(trackedKeywordsMessagesGroup);
        ...
        resetGeneralSettingsButton.addEventListener('click', () => {
            if (confirm("Are you sure you want to reset all general settings to default?")) {
                ...
                document.getElementById('otk-tracked-keywords-messages-input').value = "otk";
        ...
        evenMessagesSection.appendChild(createThemeOptionRow({ labelText: "Background Colour:", storageKey: 'msgDepthOddBgColor', cssVariable: '--otk-msg-depth-odd-bg-color', defaultValue: '#ffffff', inputType: 'color', idSuffix: 'msg-depth-odd-bg', requiresRerender: true }));
        evenMessagesSection.appendChild(createThemeOptionRow({ labelText: "Own Post Background Colour:", storageKey: 'ownMsgBgColorOdd', cssVariable: '--otk-own-msg-bg-color-odd', defaultValue: '#d1e7ff', inputType: 'color', idSuffix: 'own-msg-bg-odd', requiresRerender: true }));
        evenMessagesSection.appendChild(createThemeOptionRow({ labelText: "Own Post Border Colour:", storageKey: 'ownMsgBorderColorOdd', cssVariable: '--otk-own-msg-border-color-odd', defaultValue: '#c1d7ef', inputType: 'color', idSuffix: 'own-msg-border-odd', requiresRerender: true }));
        evenMessagesSection.appendChild(createThemeOptionRow({ labelText: "Media Controls BG (Odd):", storageKey: 'mediaControlsBgColorOdd', cssVariable: '--otk-media-controls-bg-color-odd', defaultValue: 'rgba(255, 255, 255, 0.8)', inputType: 'color', idSuffix: 'media-controls-bg-odd' }));
        evenMessagesSection.appendChild(createThemeOptionRow({ labelText: "Filter Icon:", storageKey: 'blockIconColorOdd', cssVariable: '--otk-block-icon-color-odd', defaultValue: '#999999', inputType: 'color', idSuffix: 'block-icon-odd' }));
        evenMessagesSection.appendChild(createThemeOptionRow({ labelText: "Pin Icon:", storageKey: 'pinIconColorOdd', cssVariable: '--otk-pin-icon-color-odd', defaultValue: '#666666', inputType: 'color', idSuffix: 'pin-icon-odd' }));
        evenMessagesSection.appendChild(createThemeOptionRow({ labelText: "Pin Icon (Active):", storageKey: 'pinIconColorActiveOdd', cssVariable: '--otk-pin-icon-color-active-odd', defaultValue: '#ffffff', inputType: 'color', idSuffix: 'pin-icon-active-odd' }));
        evenMessagesSection.appendChild(createThemeOptionRow({ labelText: "New Message — Top Padding:", storageKey: 'otkNewMessagePaddingTop', defaultValue: '5px', inputType: 'number', unit: 'px', min: 0, max: 100, idSuffix: 'new-message-padding-top', requiresRerender: true }));
        evenMessagesSection.appendChild(createThemeOptionRow({ labelText: "New Message — Space Below Quote Block:", storageKey: 'otkNewMessageSpaceBelowQuoteBlock', defaultValue: '11px', inputType: 'number', unit: 'px', min: 0, max: 100, idSuffix: 'new-message-space-below-quote-block', requiresRerender: true }));
        evenMessagesSection.appendChild(createThemeOptionRow({ labelText: "New Message — Bottom Padding:", storageKey: 'otkNewMessagePaddingBottom', defaultValue: '11px', inputType: 'number', unit: 'px', min: 0, max: 100, idSuffix: 'new-message-padding-bottom', requiresRerender: true }));
        ...
        oddMessagesSection.appendChild(createThemeOptionRow({ labelText: "Background Colour:", storageKey: 'msgDepthEvenBgColor', cssVariable: '--otk-msg-depth-even-bg-color', defaultValue: '#d9d9d9', inputType: 'color', idSuffix: 'msg-depth-even-bg', requiresRerender: true }));
        oddMessagesSection.appendChild(createThemeOptionRow({ labelText: "Own Post Background Colour:", storageKey: 'ownMsgBgColorEven', cssVariable: '--otk-own-msg-bg-color-even', defaultValue: '#c1d7ef', inputType: 'color', idSuffix: 'own-msg-bg-even', requiresRerender: true }));
        oddMessagesSection.appendChild(createThemeOptionRow({ labelText: "Own Post Border Colour:", storageKey: 'ownMsgBorderColorEven', cssVariable: '--otk-own-msg-border-color-even', defaultValue: '#c1d7ef', inputType: 'color', idSuffix: 'own-msg-border-even', requiresRerender: true }));
        oddMessagesSection.appendChild(createThemeOptionRow({ labelText: "Media Controls BG (Even):", storageKey: 'mediaControlsBgColorEven', cssVariable: '--otk-media-controls-bg-color-even', defaultValue: 'rgba(217, 217, 217, 0.8)', inputType: 'color', idSuffix: 'media-controls-bg-even' }));
        oddMessagesSection.appendChild(createThemeOptionRow({ labelText: "Media Menu Icon Colour:", storageKey: 'mediaMenuIconColor', cssVariable: '--otk-media-menu-icon-color', defaultValue: '#ff8040', inputType: 'color', idSuffix: 'media-menu-icon' }));
        oddMessagesSection.appendChild(createThemeOptionRow({ labelText: "Filter Icon:", storageKey: 'blockIconColorEven', cssVariable: '--otk-block-icon-color-even', defaultValue: '#999999', inputType: 'color', idSuffix: 'block-icon-even' }));
        oddMessagesSection.appendChild(createThemeOptionRow({ labelText: "Pin Icon:", storageKey: 'pinIconColorEven', cssVariable: '--otk-pin-icon-color-even', defaultValue: '#666666', inputType: 'color', idSuffix: 'pin-icon-even' }));
        oddMessagesSection.appendChild(createThemeOptionRow({ labelText: "Pin Icon (Active):", storageKey: 'pinIconColorActiveEven', cssVariable: '--otk-pin-icon-color-active-even', defaultValue: '#ffffff', inputType: 'color', idSuffix: 'pin-icon-active-even' }));
        oddMessagesSection.appendChild(createThemeOptionRow({ labelText: "Quoted Messages — Outer Padding:", storageKey: 'otkQuotedOuterPadding', defaultValue: '11px', inputType: 'number', unit: 'px', min: 0, max: 100, idSuffix: 'quoted-outer-padding', requiresRerender: true }));
        oddMessagesSection.appendChild(createThemeOptionRow({ labelText: "Quoted Messages — Internal Padding:", storageKey: 'otkQuotedInternalPadding', defaultValue: '5px', inputType: 'number', unit: 'px', min: 0, max: 100, idSuffix: 'quoted-internal-padding', requiresRerender: true }));
        oddMessagesSection.appendChild(createThemeOptionRow({ labelText: "Quoted Message — Header Spacing:", storageKey: 'otkQuotedHeaderSpacing', defaultValue: '1px', inputType: 'number', unit: 'px', min: 0, max: 100, idSuffix: 'quoted-header-spacing', requiresRerender: true }));
        oddMessagesSection.appendChild(createThemeOptionRow({ labelText: "Quoted Messages — Divider Spacing:", storageKey: 'otkQuotedDividerSpacing', defaultValue: '5px', inputType: 'number', unit: 'px', min: 0, max: 100, idSuffix: 'quoted-divider-spacing', requiresRerender: true }));
        oddMessagesSection.appendChild(createThemeOptionRow({ labelText: "Divider Line Colour:", storageKey: 'otkQuotedMessageDividerColor', defaultValue: '#ff8040', inputType: 'color', idSuffix: 'quoted-message-divider-color', requiresRerender: true }));
        oddMessagesSection.appendChild(createDropdownRow({ labelText: "Divider Line Style:", storageKey: 'otkQuotedMessageDividerStyle', options: ['dashed', 'solid', 'dotted', 'none'], defaultValue: 'dashed', requiresRerender: true }));
...
function applyScrollButtonPosition() {
    const themeSettings = JSON.parse(localStorage.getItem(THEME_SETTINGS_KEY)) || {};
    const position = themeSettings.otkScrollButtonPosition || 'Bottom of Page';
    const scrollButtonContainer = document.getElementById('otk-scroll-button-container');
...
function getAllOptionConfigs() {
            ...
            return [
                ...
                { storageKey: 'otkNewMessagePaddingTop', defaultValue: '5px', inputType: 'number', unit: 'px', min: 0, max: 100, idSuffix: 'new-message-padding-top' },
                { storageKey: 'otkNewMessageSpaceBelowQuoteBlock', defaultValue: '11px', inputType: 'number', unit: 'px', min: 0, max: 100, idSuffix: 'new-message-space-below-quote-block' },
                { storageKey: 'otkNewMessagePaddingBottom', defaultValue: '11px', inputType: 'number', unit: 'px', min: 0, max: 100, idSuffix: 'new-message-padding-bottom' },
                { storageKey: 'otkQuotedOuterPadding', defaultValue: '11px', inputType: 'number', unit: 'px', min: 0, max: 100, idSuffix: 'quoted-outer-padding' },
                { storageKey: 'otkQuotedInternalPadding', defaultValue: '5px', inputType: 'number', unit: 'px', min: 0, max: 100, idSuffix: 'quoted-internal-padding' },
...
function setupFilterWindow() {
    ...
    function applyDefaultSettings() {
        const defaults = {
            ...
            "otkTrackedKeywordsMessages": "otk",
...
function setupScrollButtons() {
    ...
    async function main() {
        ...
        try {
            ...
            await recalculateAndStoreMediaStats(); // This updates localStorage
                ...
                let savedMode = localStorage.getItem('otkViewerMode');
                if (!savedMode) {
                    const isOpen = localStorage.getItem(VIEWER_OPEN_KEY);
                    savedMode = isOpen === 'true' ? 'default' : 'closed';
                    localStorage.setItem('otkViewerMode', savedMode);
                }

                updateToggleButtonText();
                updateViewerTitle();

                if (savedMode !== 'closed' && otkViewer) {
                    otkViewer.classList.add('otk-message-layout-default');
                    otkViewer.classList.remove('otk-message-layout-newdesign');
                    consoleLog(`Viewer state restored to ${savedMode}. Layout class applied. Rendering.`);
                    otkViewer.style.display = 'block';
                    if (scrollButtonContainer) scrollButtonContainer.style.display = 'flex';
                    document.body.style.overflow = 'hidden';
    ...
    saveToSystemButton.addEventListener('click', async () => {
        ...
        const allSettings = {};
        const keysToExport = [
            OTK_TRACKED_KEYWORDS_KEY, 'otkTrackedKeywordsMessages', OTK_BLOCKED_KEYWORDS_KEY, 'otkMinUpdateSeconds',
...
function setupTimezoneSearch() {
    ...
    // Expose globals for automated testing and verification (inside init scope)
    ...
    Object.defineProperty(window, 'userPostIds', {
        get: () => userPostIds,
        set: (val) => { userPostIds = val; },
        configurable: true
    });
    Object.defineProperty(window, 'unreadIds', {
        get: () => unreadIds,
        set: (val) => { unreadIds = val; },
        configurable: true
    });
    ...
    // Expose globals for automated testing and verification
